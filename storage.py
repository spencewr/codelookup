...
class SASGeneratorApp(tb.Window):
    ...
    def save_current_variable(self):
        """Save current form data locally (even if incomplete)"""
        # Get level names from entries
        level_names = []
        for entry in self.level_name_entries:
            level_names.append(entry.get().strip())

        data = {
            "dataset": self.dataset_var.get(),
            "dataset_name": SURVEYS[self.dataset_var.get()]["full_name"] if self.dataset_var.get() in SURVEYS else "",
            "population": SURVEYS[self.dataset_var.get()]["population"] if self.dataset_var.get() in SURVEYS else "",
            "tag_suffix": SURVEYS[self.dataset_var.get()]["tag_suffix"] if self.dataset_var.get() in SURVEYS else "",
            ...
            "levels": level_names,
            "levels_count": self.levels_var.get(),
        }
    ...
    def validate_current_variable(self):
        """Validate that all required fields are filled for SAS generation"""
        if not self.dataset_var.get():
            return False, "Please select a Survey Dataset."
        if not self.var_code_entry.get().strip():
            return False, "Please enter Variable Code."
        if not self.var_name_entry.get().strip():
            return False, "Please enter Variable Name."
        if not self.description_entry.get().strip():
            return False, "Please enter Description."
        if not self.var_type_var.get():
            return False, "Please select Variable Type."
        if self.var_type_var.get() == "Indicator":
            if not self.topic_var.get() or not self.subtopic_var.get():
                return False, "Please select Topic and Sub-Topic for Indicators."
        if not self.levels_var.get():
            return False, "Please select Number of Levels."
        for idx, entry in enumerate(self.level_name_entries, start=1):
            if not entry.get().strip():
                return False, f"Please enter a name for Level {idx}."
        return True, ""
    ...
    def load_variable(self, index):
        ...
        # Handle levels - use stored count if available, otherwise count from levels list
        levels_count = var_data.get("levels_count", str(len(var_data["levels"])))
        if levels_count:
            self.levels_var.set(levels_count)
            self.on_levels_change()

        # Load level names (pad with empty strings if needed)
        levels = var_data["levels"]
        for i, entry in enumerate(self.level_name_entries):
            if i < len(levels):
                entry.delete(0, "end")
    ...
    def prev_variable(self):
        if self.current_var_index > 0:
            # Always save current state (even if incomplete)
    ...
    def update_nav_buttons(self):
        ...
        # Always allow delete since we can clear the form
        self.delete_var_btn.configure(state="normal")

    def delete_variable(self):
        # If we're editing a new variable (not saved yet) or no variables exist
        if self.current_var_index >= len(self.variables) or not self.variables:
            # Just clear the form
            ...
            if result:
                self.clear_form()
                # If we were beyond the saved variables, go back to the last saved one
                if self.variables:
                    self.current_var_index = len(self.variables) - 1
        ...
        # We're deleting a saved variable
        var_name = self.variables[self.current_var_index].get("var_name", "Unknown")
        if not var_name:
            var_name = f"Variable {self.current_var_index + 1}"
        ...
        self.update_nav_buttons()
        messagebox.showinfo("Success", f"'{var_name}' has been deleted.")
    ...
    def generate_sas_code(self):
        ...
        for i, var_data in enumerate(self.variables):
            # Temporarily set the variable data to validate it
            temp_dataset = var_data["dataset"]
            temp_var_code = var_data["var_code"]
            temp_var_name = var_data["var_name"]
            temp_description = var_data["description"]
            temp_var_type = var_data["var_type"]
            temp_topic = var_data["topic"]
            temp_sub_topic = var_data["sub_topic"]
            temp_levels = var_data["levels"]
            
            # Check if variable is complete
            is_valid = True
            if not temp_dataset or not temp_var_code or not temp_var_name or not temp_description or not temp_var_type:
                is_valid = False
            if temp_var_type == "Indicator" and (not temp_topic or not temp_sub_topic):
                is_valid = False
            if not temp_levels or any(not level.strip() for level in temp_levels):
                is_valid = False
                
            if not is_valid:
                var_name = temp_var_name if temp_var_name else f"Variable {i + 1}"
                invalid_variables.append(var_name)

        if invalid_variables:
            messagebox.showerror(
                "Incomplete Variables", 
                f"The following variables are incomplete and cannot be used to generate SAS code:\n\n" +
                "\n".join(f"• {var}" for var in invalid_variables) +
                "\n\nPlease complete all required fields for each variable."
            )
            return
        ...
        for var_data in self.variables:
            for idx, val in enumerate(var_data["levels"], start=1):
                if val.strip():  # Only include non-empty level names
